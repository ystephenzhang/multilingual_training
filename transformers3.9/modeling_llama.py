...
class LlamaMLP(nn.Module):
    ...
    def forward(self, x, early_layers = None, deactivate_down = None, deactivate_up = None):
        if deactivate_up:
            for fwd_up_index in deactivate_up:
                self.up_proj.weight[fwd_up_index, :] = 0
        if deactivate_down:
            for fwd_down_index in deactivate_down:
                self.down_proj.weight[:,fwd_down_index] = 0
        
        down_proj = self.down_proj(self.act_fn(self.gate_proj(x)) * self.up_proj(x))
        #!
        if early_layers:
            return down_proj, torch.sum(torch.abs(self.up_proj(x)), dim=1).squeeze().tolist(), torch.sum(torch.abs(self.up_proj(x)), dim=1).squeeze().tolist()
...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    #!
    #print("Attention Weight activation: states shapes", query, key_states)
    if early_layers:
        attn_weights_temp = torch.matmul(query.transpose(2,3).unsqueeze(-1), key_states.transpose(2,3).unsqueeze(-1).transpose(-2,-1)) * scaling
        attn_weights_temp = attn_weights.unsqueeze(2).expand(-1,-1,query.size()[-1],-1,-1) - attn_weights_temp
        attn_weights_temp = nn.functional.softmax(attn_weights_temp, dim=-1, dtype=torch.float32).to(query.dtype)
    #!
    ...
    #!
    if early_layers:
        attn_weights_temp = attn_weights_temp - attn_weights.unsqueeze(2).expand(-1,-1,query.size()[-1],-1,-1)
        attn_weights_temp = attn_weights_temp**2
        attn_weights_temp = attn_weights_temp.sum(dim=(-2, -1)).view(-1)
    #!
    #!
        bsz, q_len, _, _ = hidden_shape
        attn_output_temp = attn_output.reshape(bsz, q_len, -1)
    #!

        return attn_output, attn_weights, attn_weights_temp.squeeze().tolist(), attn_weights_temp.squeeze().tolist(), attn_output_temp, torch.sum(torch.abs(attn_output_temp), dim=1).squeeze().tolist() #!
    else:
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if deactivate_keys_k:
            for k_index in deactivate_keys_k:
                self.k_proj.weight[k_index // 4, :] = 0
        if deactivate_keys_v:
            for v_index in deactivate_keys_v:
                self.v_proj.weight[v_index // 4, :] = 0
        ...
        # !: disabling sdpa attention  
        '''
        if self.config._attn_implementation != "eager" and not early_layers:
        ...
        #!
        if early_layers:
            attn_output, attn_weights, query_score, key_score, attn_output_temp, o_score = attention_interface(
                self,
                hidden_shape,
                query_states,
                key_states,
        ...
        #!
            return attn_output, attn_weights, query_score, key_score, torch.sum(torch.abs(attn_output_temp), dim=1).squeeze().tolist(), o_score
        else:
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Self Attention
        hidden_states, self_attn_weights, q_score, k_score, v_score, o_score = self.self_attn(
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        self,
        early_exit_layers: Optional[List[int]],
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        #!
        index_keys = range(32)
        index_keys_under = range(under_layer) if under_layer else []
        index_keys_gen = [31-i for i in range(gen_layer)] if gen_layer else []
        if not under_layer and not gen_layer:
            index_keys_reason = []
        else:
        ...
        for idx, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            ...
            if early_exit:
                hidden_scores_fwd_up[idx] = up_score
                hidden_scores_fwd_down[idx] = down_score
                hidden_scores_q[idx] = q_score
                hidden_scores_k[idx] = k_score
                hidden_scores_v[idx] = v_score
        ...
        if return_dict:
            return output, hidden_scores_fwd_up, hidden_scores_fwd_down, hidden_scores_q, hidden_scores_k, hidden_scores_v, hidden_scores_o
        else:
...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        outputs, hidden_scores_fwd_up, hidden_scores_fwd_down, hidden_scores_q, hidden_scores_k, hidden_scores_v, hidden_scores_o = self.model(
            input_ids=input_ids,
        ...
        if early_exit_layers is not None:
            ...
            for i, early_exit_layer in enumerate(early_exit_layers):
                ...
                top_indices = np.argsort(hidden_scores_q[early_exit_layer])[-top_number_attn:][::-1]
                activate_keys_q[early_exit_layer] = top_indices
                top_indices = np.argsort(hidden_scores_k[early_exit_layer])[-top_number_attn:][::-1]
                activate_keys_k[early_exit_layer] = top_indices
                top_indices = np.argsort(hidden_scores_v[early_exit_layer])[-top_number_attn:][::-1]
                activate_keys_v[early_exit_layer] = top_indices
                top_indices = np.argsort(hidden_scores_o[early_exit_layer])[-top_number_attn:][::-1]
