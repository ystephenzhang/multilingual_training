...
class SwiftSft(SwiftPipeline, TunerMixin):
    ...
    def _read_neuron(self, path, layers_strat, types_strat, top_k = -1):
        layers_mapping = {
            "all": range(32),
            "front": range(10),
            "middle": range(10, 20),
    ...
    def run(self):
        ...
        self.args.training_args.activate_neuron = self._read_neuron(self.args.activate_path,
                                                                    self.args.activate_layers,
                                                                    self.args.activate_types) if self.args.activate_path else None
        self.args.training_args.log_grad = self.args.log_grad        
